...
Returns the low and high indices as well, so putting in a `P high low` completes the knot.
-/
def processGaussCode [Inhabited α] [DecidableEq α]
  (code : Array (GaussEntry α)) : Option (PD Nat × Nat × Nat) := Id.run
do
...
def parseFile (fname : System.FilePath) : IO (Array (String × PD Nat × Nat × Nat)) := do
  ...
  let mut crossings := 0
  let mut idx := 1
  let mut knots : Array (String × PD Nat × Nat × Nat) := #[]
  for s in ← IO.FS.lines fname do
    ...
    | .ok code => do
      let some (pd, bdry) := processGaussCode code | throw (IO.userError "Invalid Gauss code")
      if pd.crossings ≠ crossings then
        crossings := pd.crossings
        idx := 1
      let name := s!"{crossings}_{idx}"
      knots := knots |>.push (name, pd, bdry)
      idx := idx + 1
    line := line + 1
  return knots
