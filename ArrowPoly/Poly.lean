...
-- Should have proof that the Monomial.exponents are in strictly increasing order
def Poly := Array Monomial deriving Repr

def Poly.mk (a : Array Monomial) : Poly := a

def Poly.toArray (p : Poly) : Array Monomial := p
...
def Monomial.mul (m₁ m₂ : Monomial) : Monomial where
  ...
  exponents := (Array.zipWith' m₁.exponents m₂.exponents (. + .)).dropZeros
  coeff_nonzero := by
    intro h
    cases Int.eq_zero_or_eq_zero_of_mul_eq_zero _ _ h
    apply m₁.coeff_nonzero; assumption
    apply m₂.coeff_nonzero; assumption
  exp_norm := Array.back?_dropZeros _

def Poly.add (p₁ p₂ : Poly) : Poly :=
  let f (p : Poly) : Merge Monomial → Poly
    | .left m₁ => p.push m₁ 
    | .right m₂ => p.push m₂
    | .both m₁ m₂ =>
  ...
  Array.mergeBy Monomial.exponents f p₁ p₂

instance : Add Poly where
  add := Poly.add

def Poly.sub (p₁ p₂ : Poly) : Poly :=
  let f p : Merge Monomial → Poly
    | .left m₁ => p.push m₁ 
...
def Poly.mulScalar (p : Poly) (c : Int) (hc : c ≠ 0) : Poly :=
  p.map (λ m => m.scale c hc)

instance : HMul Int Poly Poly where
  hMul c p := if h : c = 0 then #[] else p.mulScalar c h
...
instance : Coe Int Poly where
  coe n := if h : n = 0 then #[] else #[Monomial.incl n h]
...
def Poly.A (exp : Int := 1) : Poly :=
  if he : exp = 0 then Monomial.incl 1
          ...
          exponents := #[exp],
          exp_norm := by { show some exp ≠ some 0; simp [he] }}]

def Poly.K (i : Nat) (exp : Nat := 1) : Poly :=
  if he : exp = 0 then Monomial.incl 1
  else #[{coeff := 1, exponents := (Array.mkArray (i+1) 0).push exp}]
